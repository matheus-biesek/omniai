...
| **UsageRecord** | `id`, `projectId`, `provider`, `model`, `promptTokens`, `completionTokens`, `totalTokens`, `costUsd`, `latencyMs`, `status`, `occurredAt` | Um registro por chamada de IA reportada pelo SDK. É a tabela de fatos do sistema — todo agregado exibido no dashboard é calculado a partir dela. |
